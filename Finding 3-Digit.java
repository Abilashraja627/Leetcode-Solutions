        ...
        for (int first = 0; first < digits.length; first++) {
            ...
            for (int second = 0; second < digits.length; second++) {
                ...
                for (int third = 0; third < digits.length; third++) {
                    if (digits[third] % 2 == 0 && third != first && third != second) {
                        int t = digits[third];
                        list.add((f * 100) + (digits[second] * 10) + t);
                    }
                }
            }
        }
        
        int[] result = list.stream().mapToInt(i -> i).distinct().toArray();
        Arrays.sort(result);
        return result;
    }
}
